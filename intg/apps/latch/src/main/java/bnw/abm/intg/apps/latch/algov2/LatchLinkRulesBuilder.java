...
public class LatchLinkRulesBuilder extends LinkRulesBuilder {

    @Override
    public void build() {
        int agentTypesCount = 96;
        int ageCatsCount = 8;
        ...
        for (int i = 0; i < agentTypesCount; i++) {
            ...
            if ((AT.ST_M_MAR.getCatId() <= i & i <= AT.LS_M_MAR.getCatId()) || (AT.ST_F_MAR.getCatId() <= i & i <= AT.LS_F_MAR.getCatId()
                ...
                // Parental relationships for - (Married and Lone Parents)
                targets = new ArrayList<>();
                int fromOffset = (i % ageCatsCount) + 1, toOffset = (i % ageCatsCount) + 3, from, to;
            ...
            if ((AT.ST_M_CHLD.getCatId() <= i & i <= AT.LS_F_CHLD.getCatId()) // Children male and female
                ...
                targets = new ArrayList<>();
                // A parent must not be older than 3 age categories than child's age category. And parent must be at least one age
                // category older than the child.
                int fromOffset = (i % ageCatsCount) - 3, toOffset = (i % ageCatsCount) - 1, from, to;
