...
enum FamilyType {
    ...
    private FamilyType(String familyType, int priority) {
...
enum HouseholdSize {
    ...
    HouseholdSize(String personsCountStr, int oersonsCount) {
...
public class IPUDataGroupType extends GroupTypeLogic {
    static List<String> groupTypes = new ArrayList<>(10);

    public IPUDataGroupType() {
        List<String> familyTypes = new ArrayList<>(10);
    ...
    static List<FamilyUnit> computeFamilyUnits(GroupTemplate groupTemplate) {
        if (groupTemplate.size() == 1) {
            GroupTemplate.Member member = groupTemplate.getCurrentReferenceMembers().get(0);
        ...
        GroupTemplate.Member groupHhMember = groupTemplate.getCurrentReferenceMembers().get(0);
        ...
        List<GroupTemplate.Member> allMembers = new ArrayList<>(groupTemplate.getAllMembers());
        ...
        while (allMembers.size() > 0 && groupTemplate.size() != checkedGroupHeadsCount) {// Stop when we have tried all members
            ...
            GroupTemplate.Member head = allMembers.get(0);
            checkedGroupHeadsCount++;
            if (AT.ST_M_MAR.getCatId() <= head.getTypeID() && head.getTypeID() <= AT.LS_F_MAR.getCatId()) {
                List<GroupTemplate.Member> partners = null, couplesChildren = null;
                ...
                if (partners.size() == 1) {
                    ...
                    List<GroupTemplate.Member> relativesOf1 = groupTemplate.getAdjacentMembers(head, MyLink.FAMILY_RELATIVE.getLink());
                    ...
                    List<GroupTemplate.Member> relativesOf2 = groupTemplate.getAdjacentMembers(partners.get(0),
            ...
            if (AT.ST_M_LNPAR.getCatId() <= head.getTypeID() && head.getTypeID() <= AT.LS_F_LNPAR.getCatId()) {
                List<GroupTemplate.Member> dependentChildren = null;
            ...
            if (AT.ST_REL.getCatId() <= head.getTypeID() && head.getTypeID() <= AT.LS_REL.getCatId()) {
                List<GroupTemplate.Member> otherFamilyRelative = groupTemplate.getAdjacentMembers(head, MyLink.RELATIVE_OTHER.getLink());
    ...
    public GroupType computeGroupType(GroupTemplate groupTemplate) {
        ...
        else if (groupTemplate.size() == 7 || groupTemplate.size() == 8) {
            householdSize = HouseholdSize.P6;
        } else
            return GroupType.getInstance(groupTypes.indexOf("Invalid"));
...
class FamilyUnit {
    ...
    FamilyUnit(GroupTemplate.Member head) {
        this.headType = head.getTypeID();
    }

    void addPartner(GroupTemplate.Member partner) {
        if (this.partnerType == -1)
            this.partnerType = partner.getTypeID();
        else
            throw new UnsupportedOperationException("Partner already exists");
    }

    void addOtherMembers(List<GroupTemplate.Member> others) {
        for (GroupTemplate.Member member : others) {
